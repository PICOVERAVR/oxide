...
pub fn read_cfg(path: &str) -> Option<Triple> {
    ...
    let cfg_base = cfg.try_into::<toml::map::Map<String, Value>>().expect("could not cast into map");
    ...
    for (k, v) in cfg_base {
        match k.as_str() {
            "materials" => {
                let mat_map = v.try_into::<toml::map::Map<String, Value>>().expect("could not cast material into map");
            ...
            "lights" => {
                let light_map = v.try_into::<toml::map::Map<String, Value>>().expect("could not cast light into map");
            ...
            "render" => c.render = Render {
                max_reflections: v["max_reflections"].as_integer().expect("could not cast into integer") as u32,
                threads: v["threads"].as_integer().expect("could not cast into integer") as u32,
            },
            "output" => c.output = Output {
                width: v["width"].as_integer().expect("could not cast into integer") as usize,
                height: v["height"].as_integer().expect("could not cast into integer") as usize,
